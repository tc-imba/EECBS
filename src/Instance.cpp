...
Instance::Instance(const string &map_fname, const string &agent_fname,
    ...
    dynamic_obstacles_constraints = ConstraintTable(num_of_cols, map_size);
}
...
void Instance::loadDynamicObstacles(const std::string &dynamic_obstacles_fname) {
//    vector<tuple<size_t, int, int>> dynamic_obstacles;
    dynamic_obstacles_constraints.clear();
//    std::cout << dynamic_obstacles_fname << std::endl;

    std::ifstream myfile(dynamic_obstacles_fname.c_str());
    if (!myfile.is_open()) return;

    int length;
    myfile >> length;
    for (int i = 0; i < length; i++) {
        int row, col, t_min, t_max;
        myfile >> col >> row >> t_min >> t_max;
        auto index = linearizeCoordinate(row, col);
//        dynamic_obstacles.emplace_back(index, t_min, t_max);
//        std::cout << index << " " << t_min << " " << t_max << std::endl;
        dynamic_obstacles_constraints.insert2CT(index, t_min, t_max);
    }
}